...
- [`ffgac`](#ffgac) is just `ffmpeg`, but with some extra features for glitch artists.
- [`ffedit`](#ffedit) is the main tool for FFglitch. It is a **multimedia bitstream editor**.
- [`ffplay`](#ffplay) is a video player that integrates `ffedit` so you can create live glitch in real-time.
